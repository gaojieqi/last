...
decay_rate=0.7#parameter of repair operator
...
def GA(mut=0.8,crossp=0.7,popsize=population,its_GA=2,its_EADA=2):
    ...
    print fitness #return fitness, len(fitness)==popsize
...
#inner iteration
def EADA(structure_info, mut=0.8, crossp=0.7, popsize=1, its=2):
    pop = []
    fitness=[]
        ...
        for kk in range(Ns):
            ...
            if kk != Ns - 1:
                ...
                for ii in range(Nc):
                    ...
                    T[kk][2][ii] = cold[ii][0] + float(heat_load[kk][ii]) / cold[ii][3]
            ...
            if kk == 0:
                for jj in range(Nh):
                    for ii in range(Nc):
                        if structure_info[Nh*Nc*kk+ii * Nh + jj] == 1:
                            T[kk][3][jj] += (float(heat_load[kk][ii] * split[kk][ii][jj]) / hot[jj][3])
            if kk!=0 and kk!=Ns-1:
                for jj in range(Nh):
                    T[kk][1][jj] = T[kk - 1][3][jj]
                    for ii in range(Nc):
    ...
    for asdf in range(its):
        ...
        for ppppp in range(popsize):
            ...
            buf_pa=copy.deepcopy(np.array(getnewList(pa)))
            buf_pb=copy.deepcopy(np.array(getnewList(pb)))
            differ=copy.deepcopy(CF*(buf_pb-buf_pa))
            differ=getlist(differ)
            differ=mutation(differ,mut)
            differ=np.array(differ)
            #mutation
            buf_pop=copy.deepcopy(np.array(getnewList(pop[ppppp])))
            differ=np.array(getnewList(differ))
            buf_pop+=differ+buf_pop
            pop[ppppp]=getlist(buf_pop)
            #repair split
            for kk in range(Ns):
                if kk!=Ns-1:
                    for ii in range(Nc):
            ...
            for jj in range(Nh):
                ...
                for ii in range(Nc):
                    if structure_info[Nh * Nc * kk + ii * Nh + jj] == 1:
                        ...
                        f__=1
                if f__==1:
                    tttt = T_[1][jj] + d / float(cold[ii][3])
                    T__.append(tttt )
                if f__==0:
                    T__.append(T_[1][jj])
            T_.append([T__])
                        ...
                        for jj in range(Nh):
                            if structure_info[Nh*Nc*kk+ii * Nh + jj] == 1:
                                if t[kk][3][jj] - delta_T < t[kk][2][ii] or t[kk][2][ii]>cold[ii][1]:
                                    ll = (hot[jj][2] - cold_utility[jj]) / 4
                                    summmm[jj] += ll
                                    cold_utility[jj] += ll
                                    heat_load[kk][jj] -= ll
                                    for iii in range(Nc):
                                        if structure_info[Nh*Nc*kk+iii * Nh + jj] == 1:
                                            t[kk][2][iii] =t[kk][0][iii]+ heat_load[kk][jj] * sp[kk][jj][iii] / float(cold[iii][3])
                                    flag = 0
                                    global_flag=0
                        if ii == Nc - 1 and flag == 1:
                            stop = 1
                for kkk in range(Ns):
                    for jj in range(Nh):
...
def getnewList(newlist):
    ...
    for element in newlist:
        if not isinstance(element, list):
            ...
            d.extend([np.asarray(getnewList(element),object)])
    return d
def getlist(newarray):
    d=[]
    na=newarray.tolist()
    for element in na:
        if not isinstance(element, np.ndarray):
            d.append(element.tolist())
        else:
            d.extend([getlist(element)])
    return d
def mutation(li,mut):
    d = []
    for element in li:
